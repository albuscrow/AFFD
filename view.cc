...
View::View(QWidget *parent, CommonData *commonData) : QGLWidget(desiredFormat()) {
    //fout.open("View.txt");
...
void View::setBufferData() {
	...
	viewMemD += sizeof(float) * tessPointCount * 3;
	printMemD(__FILE__, __FUNCTION__, __LINE__, sizeof(float) * tessPointCount * 3, "@法向VBO_truth");

	glBindBuffer(GL_ARRAY_BUFFER, vertexVBO_truth);
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * tessPointCount * 3, 0, GL_DYNAMIC_DRAW);
	//cout << "@顶点坐标VBO_truth" << endl << "\t";
	viewMemD += sizeof(float) * tessPointCount * 3;
	printMemD(__FILE__, __FUNCTION__, __LINE__, sizeof(float) * tessPointCount * 3, "@顶点坐标VBO_truth");
#endif
...
void View::paintGL()                                        // 完成opengl绘制的函数
{
    ...
    if (m_bLoaded)                                            // 如果obj文件已经载入
    {
        ...
        m_modelViewMatrixStack.rotate(m_fTheta, rotateAxis[0], rotateAxis[1],
                                      rotateAxis[2]);    // 计算新的旋转矩阵，即：m = e·r = r
        ...
        if ((m_nDisplayMode & DISPLAY_MODEL) != 0)            // 需要绘制模型
        {
            ...
            else                                            // AFFD算法
            {
                if (m_pCommonData->isGPU())                    // 使用GPU计算
                {
                    ...
                    glVertexAttribPointer(index_mTexCoord3D, 3, GL_FLOAT, GL_FALSE, 0, 0);
...
void View::mouseReleaseEvent(QMouseEvent *event) {
    ...
    if (event->button() == Qt::RightButton)        // 放开的是右键，此时需要计算选中的控制顶点
    {
        ...
        if (selectionWidth > ZERO && selectionHeight > ZERO) {
            ...
            gluPickMatrix((GLdouble) centerX, (GLdouble) (viewport[3] - centerY), selectionWidth, selectionHeight,
                          viewport);
            glMultMatrixf(m_projectionMatrixStack.top());
