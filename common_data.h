...
public:
    CtrlPoint(double x = 0.0, double y = 0.0, double z = 0.0, bool selected = false) : VertexCoord(x, y,
                                                                                                   z) { m_bSelected = selected; }
...
public:
    enum OnFace {
        space = 0, x_min = 1, y_min = 2, z_min = 4, x_max = 8,
        y_max = 16, z_max = 32
    };
...
class Triangle {
public:
    objdata::VertexCoord v[3], v_origin[3], samplePoint[28], barr[28], strip[28];
    int boxInfo[28][3];
...
#ifdef LINE
             const objdata::VertexCoord &bary_origin0, const objdata::VertexCoord &bary_origin1,
             const objdata::VertexCoord &bary_origin2,
#endif
             const objdata::VertexCoord *v_origin, int origin_face_idx,
             int nc0, int nc1, int nc2,
             const objdata::TextureCoord &vt0 = 0.0f, const objdata::TextureCoord &vt1 = 0.0f,
             const objdata::TextureCoord &vt2 = 0.0f);
};
...
class CommonData {
    ...
    objdata::VertexCoord BSplineVolumeValueMatrix(double u, double v, double w, int leftUIdx, int leftVIdx,
                                                  int leftWIdx);
    ...
    objdata::VertexCoord calcBaryCoord(const objdata::VertexCoord &v0, const objdata::VertexCoord &v1,
                                       const objdata::VertexCoord &v2, const objdata::VertexCoord &v);
     ...
     * 从上到下，从左到右，遍历所有三角化点
     */
    int triangleCoord(int floor, int room) const { return (1 + floor) * floor / 2 + room; }
...
public:
    ...
    /* 返回第 i 个面片的顶点数量 */
    unsigned int vertexCoordIdxCount(unsigned int i) const { return faceList[i].vertexCoordIndex.size(); }

    /* 返回第 i 个面片第 j 个顶点的编号 */
    int vertexCoordIdx(unsigned int i, unsigned int j) const { return faceList[i].vertexCoordIndex[j]; }

    int textureVertexIdx(unsigned int i, unsigned int j) const { return faceList[i].textureCoordIndex[j]; }

    /* 返回编号为 idx 的顶点 */
    objdata::VertexCoord vertexCoord(int idx) const { return vertexCoordList[idx]; }

    objdata::TextureCoord textureVertex(int idx) const { return objData->textureCoordList[idx]; }
    ...
    int ctrlPointCount(int direction) const { return m_nCtrlPointCount[direction]; }

    /* 设置 B 样条体 direction 方向（可能为U, V, W）的控制顶点数 */
    void setCtrlPointCount(int ctrlPointCount, int direction) { m_nCtrlPointCount[direction] = ctrlPointCount; }

    void cancelAllSelection();

    /* 返回相应控制顶点，i, j, k 分别为 U, V, W 方向的序号 */
    CtrlPoint getCtrlPoint(int i, int j, int k) const { return ctrlPoint[i][j][k]; }
    ...
    void setAllDirectPoint(std::vector<DirectPoint> &result) { directPointVector = result; }

    void setAllCtrlPoint(std::vector<CtrlPoint> &result);

    void setTargetCtrlPoint(std::vector<std::vector<CtrlPoint> > &, std::vector<int> &,
                            std::vector<matrix_stack::Matrix4x4> &);
    ...
    const DirectPoint &directPoint(int idx) const { return directPointVector[idx]; }
    ...
    void split(objdata::VertexCoord pCoord[], int normalCount[], std::vector<objdata::SplitResultPoint> &points,
