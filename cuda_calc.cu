...
//    }
    ...
    cout << "degree = " << degree << ", degree_lower = " << degree_lower << ", constrait_point_num = " <<
    constrait_point_num << endl;
    cout << "triangleCtrlPointNum = " << triangleCtrlPointNum << ", triangleCtrlPointNum_lower = " <<
    triangleCtrlPointNum_lower << endl;
...
float B(double u, double v, double w, int n, int3 c) {
    return factorial(n) / factorial(c.x) / factorial(c.y) / factorial(c.z) * power(u, c.x) * power(v, c.y) *
           power(w, c.z);
}
...
void generateUVW(int samplePointPerEdge) {
    ...
    printMemD(__FILE__, __FUNCTION__, __LINE__, sizeof(float) * samplePointPerTriangle * triangleCtrlPointNum_lower,
              "@第一个矩阵乘法用到的矩阵Bq存放在这里");
    ...
    printMemD(__FILE__, __FUNCTION__, __LINE__,
              sizeof(float) * samplePointPerTriangle * (triangleCtrlPointNum + constrait_point_num),
              "@第二个矩阵乘法用到的矩阵BB存放在这里");
    ...
    printMemD(__FILE__, __FUNCTION__, __LINE__, sizeof(float) * samplePointPerTriangle * triangleNum * 6,
              "@第二个矩阵乘法的结果RD存放在这里");
...
__device__ float3 BSplineVolumeValueMatrixDu(float u, float v, float w, float *shared_array,
    ...
    tv[base3 + 2] = v * v * v;
    tw[base3] = w;
    tw[base3 + 1] = w * w, tw[base3 + 2] = w * w * w;
...
__global__ void calcCtrlPoint_PN(TriangleD *triangleListD, int *triangle_adjacent_tableD, float3 *sampleValueD_PN,
                                 float *triangleCtrlPointD_PN, float *triangleNormalCtrlPointD_PN, int f, int m_) {
    ...
    int adj_corner_ctrlpoint_idx[3][2] = {{0, 2},
                                          {1, 0},
                                          {2, 1}};        // 相邻三角形0, 1, 2号边上的控制顶点编号（仅有角点，没有边点）
    for (int i = 0; i < 3; ++i)
        if (adj_face_idx[i] >= 0) {
...
__global__ void calcSampleValueThread_PN(TriangleD *triangleListD, float3 *sampleValueD_PN,
    ...
    sampleNormalD_PN[3 * triangleIdx + localIdx].x =
    ...
    sampleNormalD_PN[3 * triangleIdx + localIdx].y =
            u * J_bar_star_T_0 * x_stride + v * J_bar_star_T_1 * y_stride + w * J_bar_star_T_2 * z_stride;
    ...
    sampleNormalD_PN[3 * triangleIdx + localIdx].z =
            u * J_bar_star_T_0 * x_stride + v * J_bar_star_T_1 * y_stride + w * J_bar_star_T_2 * z_stride;
}
...
__global__ void calcConstraitSampleValueThread(TriangleD *triangleListD, float *sampleValueD,
    ...
    float J_bar_star_T_1 = fw.y * fu.z - fu.y * fw.z;
    float J_bar_star_T_2 = fu.y * fv.z - fv.y * fu.z;
    sampleNormalD[index2c(localIdx + m, triangleIdx, m + c)] =
            u * J_bar_star_T_0 * x_stride + v * J_bar_star_T_1 * y_stride + w * J_bar_star_T_2 * z_stride;
    ...
    J_bar_star_T_1 = fw.z * fu.x - fu.z * fw.x;
    J_bar_star_T_2 = fu.z * fv.x - fv.z * fu.x;
    sampleNormalD[index2c(localIdx + m, triangleIdx + f, m + c)] =
            u * J_bar_star_T_0 * x_stride + v * J_bar_star_T_1 * y_stride + w * J_bar_star_T_2 * z_stride;
    ...
    J_bar_star_T_1 = fw.x * fu.y - fu.x * fw.y;
    J_bar_star_T_2 = fu.x * fv.y - fv.x * fu.y;
    sampleNormalD[index2c(localIdx + m, triangleIdx + f * 2, m + c)] =
            u * J_bar_star_T_0 * x_stride + v * J_bar_star_T_1 * y_stride + w * J_bar_star_T_2 * z_stride;
}
...
__global__ void calcAdjustNormal(TriangleD *triangleListD, int f,
    ...
    float J_bar_star_T_1 = fw.y * fu.z - fu.y * fw.z;
    float J_bar_star_T_2 = fu.y * fv.z - fv.y * fu.z;
    triangleListD[triangleIdx].n_adj[i].x =
            u * J_bar_star_T_0 * x_stride + v * J_bar_star_T_1 * y_stride + w * J_bar_star_T_2 * z_stride;
    ...
    J_bar_star_T_1 = fw.z * fu.x - fu.z * fw.x;
    J_bar_star_T_2 = fu.z * fv.x - fv.z * fu.x;
    triangleListD[triangleIdx].n_adj[i].y =
            u * J_bar_star_T_0 * x_stride + v * J_bar_star_T_1 * y_stride + w * J_bar_star_T_2 * z_stride;
    ...
    J_bar_star_T_1 = fw.x * fu.y - fu.x * fw.y;
    J_bar_star_T_2 = fu.x * fv.y - fv.x * fu.y;
    triangleListD[triangleIdx].n_adj[i].z =
            u * J_bar_star_T_0 * x_stride + v * J_bar_star_T_1 * y_stride + w * J_bar_star_T_2 * z_stride;
}
...
__global__ void move(TriangleD *triangleListD, float *triangleCtrlPointD, int *triangle_adjacent_tableD,
    ...
    int corner_ctrlpoint_idx[6] = {9, 0, 0, 6, 6,
                                   9};                // 上面每个边控制顶点对应的角控制顶点序号(0,1号属于0号边， 2,3号属于1号边， 4,5号属于2号边)
    int oppo_corner_ctrlpoint_idx[6] = {0, 9, 6, 0, 9,
                                        6};                // 上面每个角控制顶点所在边的另一个角控制顶点编号(0,1号属于0号边， 2,3号属于1号边， 4,5号属于2号边)
    ...
    for (int i = 0; i < 6; ++i) {
        float3 v_ctrlpoint_corner = make_float3(*(p_x + corner_ctrlpoint_idx[i]), *(p_y + corner_ctrlpoint_idx[i]),
                                                *(p_z + corner_ctrlpoint_idx[i]));
        float3 v_ctrlpoint_corner_oppo = make_float3(*(p_x + oppo_corner_ctrlpoint_idx[i]),
                                                     *(p_y + oppo_corner_ctrlpoint_idx[i]),
                                                     *(p_z + oppo_corner_ctrlpoint_idx[i]));
...
void calcTriangleCtrlPoint(bool adjust_silhouette, bool use_pn, AlgorithmType algo_type) {
    if (algo_type == CYM || algo_type == LZQ) {
        ...
        cublasStatus_t stat = cublasSgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N,
                                          triangleCtrlPointNum_lower, triangleNum * 6,
                                          triangleCtrlPointNum + constrait_point_num,
                                          &alpha,
