...
//#define selectFace
	...
	/******** 进行ship模型的移动 *********/
	//moveShip(450, 449, 454, 1);
	//moveShip(453, 449, 454, 1);
	//moveShip(455, 454, 456, 0);
	//moveShip(457, 454, 456, 0);

	//moveShip(279, 280, 285, 1);
	//moveShip(283, 280, 285, 1);
	//moveShip(286, 268, 285, 0);
	//moveShip(287, 268, 285, 0);
	/******** 结束ship模型的移动 *********/
	...
	/******************** 以下代码用于输出新的兔子文件 ***********************/
	//std::map<int, int> adjacent_face_map;
	//for (vector<int>::size_type i = 0; i < border_face.size(); ++i)
	//{
		//for (vector<int>::size_type j = i + 1; j < border_face.size(); ++j)
		//{
			//for (int ii = 0; ii < 3; ++ii)
			//{
				//for (int jj = 0; jj < 3; ++jj)
				//{
					//int idx0 = faceList[border_face[i]].vertexCoordIndex[ii];
					//int idx1 = faceList[border_face[j]].vertexCoordIndex[jj];
					//VertexCoord v0 = vertexCoordList[idx0];
					//VertexCoord v1 = vertexCoordList[idx1];
					//if (idx0 != idx1 &&
						//fabs(v0.x() - v1.x()) < ZERO &&
						//fabs(v0.y() - v1.y()) < ZERO &&
						//fabs(v0.z() - v1.z()) < ZERO)
					//{
						////cout << idx0 << " <==> " << idx1 << endl;
						//int small = (idx0 < idx1 ? idx0 : idx1);
						//int big = (idx0 > idx1 ? idx0 : idx1);
						//adjacent_face_map.insert(std::make_pair(big, small));
					//}
				//}
			//}
		//}
	...
	//cout << adjacent_face_map.size() << endl;
	//for (std::map<int, int>::iterator it = adjacent_face_map.begin(); it != adjacent_face_map.end(); ++it)
	//{
		//cout << it->first + 1 << "<===>" << it->second + 1<< endl;
	//}

	//std::ifstream inFile("/home/cym/rab.obj");
	//std::ofstream frabbit("rabbit_cym.obj");
	//std::string line;
	...
	//{
		...
		//{
			...
			//char slash0, slash1;
			//frabbit << "f ";
			//while(iss >> v_id >> slash0 >> slash1 >> n_id)
			//{
				//std::map<int, int>::iterator it = adjacent_face_map.find(v_id - 1);
				//if (it != adjacent_face_map.end())
					//v_id = it->second + 1;
				//frabbit << v_id << "//" << n_id << " ";
			//}
			//frabbit << endl;
